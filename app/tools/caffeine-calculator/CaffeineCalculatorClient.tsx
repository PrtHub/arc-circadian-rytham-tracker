...
export default function CaffeineCalculatorClient() {
  ...
  const calculateCutoff = () => {
    ...
    // A cleared time input gives "", so ask for a bedtime instead of showing NaN.
    if (!targetSleep) return "Set a bedtime";

    // Pure minute arithmetic, so the server and browser render the same string.
    return formatClock(toMinutes(targetSleep) - hoursNeeded * 60);
  };
    ...
    <main className="max-w-3xl mx-auto px-6 py-14 min-h-[70vh]">
      ...
      <div className="raised-card p-6 sm:p-10 mb-12 shadow-2xl">
        <div className="grid sm:grid-cols-2 gap-8 mb-8">
          ...
          <div>
            <label htmlFor="caffeine-mg" className="block text-xs font-bold text-accent uppercase tracking-wider mb-2 font-mono">Drink Strength (mg)</label>
            <input
              id="caffeine-mg"
              type="number"
        ...
        <div className="sunken-card border border-(--accent)/30 p-8 text-center relative overflow-hidden mt-8" aria-live="polite">
          ...
          <p className="text-(--fg-muted) text-sm max-w-sm mx-auto">
            Calculated so that under 50mg of caffeine is still circulating at bedtime, for one drink of this size.
          </p>
        </div>
      </div>
      ...
      <div className="raised-card p-8 text-center border-(--accent)/30">
        ...
        <p className="text-(--fg-muted) mb-6 max-w-lg mx-auto">
          This calculator assumes one drink. ARC runs the same math on every drink you log today, allows for a realistic next cup, and shows your last safe coffee as a live countdown. It also tells you when no cup fits any more.
        </p>
