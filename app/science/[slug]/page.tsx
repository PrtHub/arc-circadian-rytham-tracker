...
export async function generateMetadata({
  ...
  return {
    ...
    openGraph: {
      title: `${article.title} | ARC Science & Protocols`,
      description: article.excerpt,
      type: "article",
...
export default async function ScienceDetailPage({
  ...
  // Extract table of contents from markdown H2s and H3s
  ...
  return (
    <div className="text-white min-h-screen relative overflow-hidden">
      ...
      {/* Focused Content Header (Left: Brand Logo, Right: Back to All Protocols) */}
      <ContentNav backHref="/science" backLabel="All Protocols" />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
          ...
          <article className="lg:col-span-8 space-y-6 text-(--fg) text-sm sm:text-base leading-relaxed">
            ...
            <CalculativeAppCallout
              variant="banner"
              badge="Scientific Automation"
              title="Apply this chronobiology research automatically"
              description="ARC calculates your exact biological phase, adenosine clearance windows, and solar lux requirements with 100% on-device precision."
              featureBullets={[
                "Real-time caffeine half-life decay tracker",
                "10,000 lux outdoor sunlight timer",
                "Lock Screen Live Activities for instant status",
              ]}
            ...
            <div className="pt-8 border-t border-white/10 flex flex-wrap items-center justify-between gap-4 font-mono text-xs text-(--fg-muted)">
              <div>
                ...
                <span className="text-white font-bold">ARC Chronobiology Lab</span>
              </div>
              <div>
                <span className="font-black uppercase tracking-widest block mb-1 opacity-70">
                  Last Updated
          ...
          <aside className="lg:col-span-4 space-y-8 lg:sticky lg:top-24">
            ...
            <CalculativeAppCallout
              badge="Protocol Tracker"
              title="ARC iOS Chrono Engine"
              description="Eliminate manual tracking. Turn peer-reviewed chronobiology into automated daily protocols."
              featureBullets={[
                "Precision circadian phase modeling",
                "Local-first SQLite storage",
                "Zero cloud sync delays",
              ]}
              location={`science_${article.slug}_sidebar`}
            />
            ...
            <div className="p-6 rounded-3xl raised-card border border-white/10">
              ...
              <div className="space-y-2 text-xs font-mono">
                ...
                <Link
                  ...
                  className="block p-2.5 rounded-xl bg-white/5 hover:bg-white/10 hover:text-accent transition-colors"
                >
                  ☀️ Solar Lux Calculator →
                </Link>
                <Link
                  href="/tools/social-jetlag-calculator"
                  className="block p-2.5 rounded-xl bg-white/5 hover:bg-white/10 hover:text-accent transition-colors"
                >
                  🧭 Social Jetlag Drift Score →
                </Link>
              </div>
            </div>
          </aside>
